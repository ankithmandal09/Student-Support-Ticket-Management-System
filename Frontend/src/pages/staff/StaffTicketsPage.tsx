...
export default function StaffTicketsPage({ assignedOnly = false }: Props) {
  ...
  const fetch = useCallback(async () => {
    ...
    try {
      ...
      if (assignedOnly && user?.id) params.assignedTo = user.id;
      const data = await listTickets(params);
      setTickets(data.items);
      setTotal(data.total);
    } catch (e) { console.error(e); }
    finally { setLoading(false); }
  }, [page, search, statusFilter, priorityFilter, assignedOnly, user?.id]);
  ...
  return (
    <DashboardLayout>
      <div className="flex items-start justify-between mb-7 gap-4 flex-wrap">
        <div>
          <h1 className="text-[1.6rem] font-extrabold text-[#f0f0fa]">{assignedOnly ? 'My Assigned Tickets' : 'All Tickets'}</h1>
          <p className="text-[#9898b8] text-[0.88rem] mt-0.5">{total} tickets</p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3 mb-5 flex-wrap">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <select id="staff-filter-status" className={SELECT} value={statusFilter} onChange={(e) => { setStatusFilter(e.target.value); setPage(1); }}>
            <option value="">All Status</option>
            {STATUSES.map(s => <option key={s} value={s}>{s.replace(/_/g, ' ')}</option>)}
          </select>
          <select id="staff-filter-priority" className={SELECT} value={priorityFilter} onChange={(e) => { setPriorityFilter(e.target.value); setPage(1); }}>
      ...
      <div className="rounded-2xl p-5 mb-5 bg-white/[0.035] border border-white/[0.08]">
        {loading ? (
          ...
          <div className="flex flex-col items-center gap-3.5 py-[60px] text-[#5f5f7a] text-center">
            ...
            <p className="text-[0.85rem]">Try adjusting your filters.</p>
          </div>
        ) : (
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between mt-4 pt-4 border-t border-white/[0.08]">
            <button id="staff-prev-page" disabled={page === 1} onClick={() => setPage(p => p - 1)} className={BTN_GHOST}>← Prev</button>
            <span className="text-[0.85rem] text-[#9898b8]">Page {page} of {totalPages}</span>
            <button id="staff-next-page" disabled={page === totalPages} onClick={() => setPage(p => p + 1)} className={BTN_GHOST}>Next →</button>
          </div>
        )}
      </div>
    </DashboardLayout>
